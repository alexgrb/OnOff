...
(function() { "use strict";
  ...
  game = {
    ...
    //function used to load our JSON
    //url :
    // callback :
    loadArea:function(url, callback) {

      var request, readyStateChange;
      //waiting for the loading of the JSON
      request = new XMLHttpRequest();

      //when finally the server returns the json level file I'm going to call the callback function
      readyStateChange = function(event) {
        //did I get back my data and did i get the responseText from my json file
        if (this.readyState == 4 && this.status == 200) {
          game.area = JSON.parse(this.responseText);
          //all the code that needs to populate the screen
          callback();
          game.engine.start();
        }
